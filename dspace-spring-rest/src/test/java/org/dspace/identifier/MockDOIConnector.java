...
public class MockDOIConnector extends MockUp<DOIConnector> implements DOIConnector {
    ...
    public void deleteDOI(Context context, String doi) throws DOIIdentifierException {
        if (reserved.remove(doi) == null) {
            throw new DOIIdentifierException("Trying to delete a DOI that was " + "never reserved!",
    ...
    public void reserveDOI(Context context, DSpaceObject dso, String doi) throws DOIIdentifierException {
        ...
        if (null != itemId) {
            if (dso.getID().equals(itemId)) {
                ...
                throw new DOIIdentifierException("Trying to reserve a DOI that " + "is reserved for another object.",
    ...
    public void registerDOI(Context context, DSpaceObject dso, String doi) throws DOIIdentifierException {
        if (!reserved.containsKey(doi)) {
            throw new DOIIdentifierException("Trying to register an unreserverd " + "DOI.",
                DOIIdentifierException.RESERVE_FIRST);
        }

        if (!reserved.get(doi).equals(dso.getID())) {
            throw new DOIIdentifierException("Trying to register a DOI that is" + " reserved for another item.",
                DOIIdentifierException.MISMATCH);
        }
        ...
        if (registered.containsKey(doi)) {
            if (registered.get(doi).equals(dso.getID())) {
                ...
                throw new DOIIdentifierException("Trying to register a DOI that " + "is registered for another item.",
                    DOIIdentifierException.MISMATCH);
            }
        }
    ...
    public void updateMetadata(Context context, DSpaceObject dso, String doi) throws DOIIdentifierException {
        if (!reserved.containsKey(doi)) {
            throw new DOIIdentifierException("Trying to update a DOI that is not " + "registered!",
                DOIIdentifierException.DOI_DOES_NOT_EXIST);
        }
        if (!reserved.get(doi).equals(dso.getID())) {
            throw new DOIIdentifierException("Trying to update metadata of an " + "unreserved DOI.",
                DOIIdentifierException.DOI_DOES_NOT_EXIST);
        }
    }
