...
public class MultipartFileSenderTest {
    ...
    public void testRangeHeader() throws Exception {
        MultipartFileSender multipartFileSender =
            MultipartFileSender.fromInputStream(is).with(requestWrapper).with(responseWrapper).withFileName(fileName)
                .withChecksum(checksum).withMimetype(mimeType).withLength(length);
    ...
    public void testNoFileName() throws Exception {
        MultipartFileSender multipartFileSender =
            MultipartFileSender.fromInputStream(is).with(requestWrapper).with(responseWrapper).withChecksum(checksum)
                .withMimetype(mimeType).withLength(length);
    ...
    public void testIfModifiedSinceNotModifiedSince() throws Exception {
        ...
        MultipartFileSender multipartFileSender =
            MultipartFileSender.fromInputStream(is).with(requestWrapper).withFileName(fileName).with(responseWrapper)
                .withChecksum(checksum).withMimetype(mimeType).withLength(length).withLastModified(time);
