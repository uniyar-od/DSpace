...
public class SearchResultMatcher {
    ...
    private static Matcher<? super Object> matchEmbeddedObject(String type) {
        return allOf(hasJsonPath("$.uuid", notNullValue()), hasJsonPath("$.name", notNullValue()),
    ...
    public static Matcher<? super Object> matchOnItemName(String type, String typePlural, String itemName) {
        return allOf(hasJsonPath("$.type", is("discover")),
            hasJsonPath("$._links.dspaceObject.href", containsString("/api/core/" + typePlural)),
            hasJsonPath("$._embedded", notNullValue()),
            hasJsonPath("$._embedded.dspaceObject", is(matchEmbeddedObjectOnItemName(type, itemName))));
    }

    private static Matcher<? super Object> matchEmbeddedObjectOnItemName(String type, String itemName) {
        return allOf(hasJsonPath("$.uuid", notNullValue()), hasJsonPath("$.name", is(itemName)),
            hasJsonPath("$.type", is(type)));
    }
    ...
    public static Matcher<? super Object> matchOnItemNameAndHitHighlight(String type, String typePlural,
        ...
        return allOf(hasJsonPath("$.type", is("discover")), hasJsonPath("$.hitHighlights",
            is(HitHighlightMatcher.entry(hitHighlightQuery, expectedFieldInHitHighlightning))),
            hasJsonPath("$._links.dspaceObject.href", containsString("/api/core/" + typePlural)),
            hasJsonPath("$._embedded", notNullValue()),
            hasJsonPath("$._embedded.dspaceObject", is(matchEmbeddedObjectOnItemName(type, itemName))));
    }
