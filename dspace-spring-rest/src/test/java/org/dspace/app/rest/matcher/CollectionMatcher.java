...
public class CollectionMatcher {
    ...
    public static Matcher<? super Object> matchCollectionEntry(String name, UUID uuid, String handle, Bitstream logo) {
        return allOf(hasJsonPath("$.uuid", is(uuid.toString())), hasJsonPath("$.name", is(name)),
            hasJsonPath("$.handle", is(handle)), hasJsonPath("$.type", is("collection")),
            hasJsonPath("$.metadata", Matchers.contains(CollectionMetadataMatcher.matchTitle(name))), matchLinks(uuid),
            matchLogo(logo));
    }
