...
public class SubmissionDefinitionsControllerIT extends AbstractControllerIntegrationTest {
    ...
    public void findDefault() throws Exception {
        getClient().perform(get("/api/config/submissiondefinitions/traditional"))
            ...
            .andExpect(jsonPath("$",
                SubmissionDefinitionsMatcher.matchSubmissionDefinition(true, "traditional", "traditional")));
    }
    ...
    public void findByCollection() throws Exception {
        ...
        getClient().perform(
            get("/api/config/submissiondefinitions/search/findByCollection").param("uuid", col1.getID().toString()))
            //** THEN **
            //The status has to be 200
            .andExpect(status().isOk()).andDo(MockMvcResultHandlers.print())
            //We expect the content type to be "application/hal+json;charset=UTF-8"
            .andExpect(content().contentType(contentType)).andExpect(
            jsonPath("$", SubmissionDefinitionsMatcher.matchSubmissionDefinition(true, "traditional", "traditional")));
    }
    ...
    public void findSections() throws Exception {
        getClient().perform(get("/api/config/submissiondefinitions/traditional/sections"))
            ...
            .andExpect(content().contentType(contentType))
            // Match only that a section exists with a submission configuration behind
            .andExpect(jsonPath("$._embedded.submissionsections", hasSize(5))).andExpect(
            jsonPath("$._embedded.submissionsections", Matchers.hasItem(
                allOf(hasJsonPath("$.id", is("traditionalpageone")),
                    hasJsonPath("$.sectionType", is("submission-form")), hasJsonPath("$.type", is("submissionsection")),
                    hasJsonPath("$._links.config.href",
                        is(REST_SERVER_URL + "config/submissionforms/traditionalpageone")),
                    hasJsonPath("$._links.self.href",
                        is(REST_SERVER_URL + "config/submissionsections/traditionalpageone"))))));
    }
}