...
public class MetadataschemaRestRepositoryIT extends AbstractControllerIntegrationTest {
    ...
    public void findAll() throws Exception {
        ...
        MetadataSchema metadataSchema =
            MetadataSchemaBuilder.createMetadataSchema(context, "ATest", "ANamespace").build();

        getClient().perform(get("/api/core/metadataschemas")).andExpect(status().isOk())
            .andExpect(content().contentType(contentType))
            .andExpect(jsonPath("$._embedded.metadataschemas", Matchers.hasItem(MetadataschemaMatcher.matchEntry())))
    ...
    public void findOne() throws Exception {
        ...
        getClient().perform(get("/api/core/metadataschemas/" + metadataSchema.getID())).andExpect(status().isOk())
            .andExpect(jsonPath("$", is(MetadataschemaMatcher.matchEntry(metadataSchema))));
    }
}
