...
public class DiscoveryRestControllerIT extends AbstractControllerIntegrationTest {
    ...
    public void discoverFacetsTestWithoutParameters() throws Exception {
        ...
        getClient().perform(get("/api/discover/facets"))
            ...
            .andExpect(jsonPath("$._embedded.facets",
                containsInAnyOrder(FacetEntryMatcher.authorFacet(false), FacetEntryMatcher.dateIssuedFacet(false),
                    FacetEntryMatcher.subjectFacet(false), FacetEntryMatcher.hasContentInOriginalBundleFacet(false))));
    }
    ...
    public void discoverFacetsAuthorTestWithSizeParameter() throws Exception {
        ...
        Item publicItem2 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2016-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withSubject("TestingForMore").withSubject("ExtraEntry")
            .build();

        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2016-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
        ...
        getClient().perform(get("/api/discover/facets/author").param("size", "2"))
            ...
            //Because we've constructed such a structure so that we have more than 2 (size) authors, there needs to
            // be a next link
            ...
            //These authors need to be in the response because it's sorted on how many times the author comes up in
            // different items
            //These authors are the most used ones. Only two show up because of the size.
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(FacetValueMatcher.entryAuthor("Doe, Jane"),
    ...
    public void discoverFacetsAuthorTestForHasMoreFalse() throws Exception {
        ...
        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2016-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        ...
        getClient().perform(get("/api/discover/facets/author"))
            ...
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/author")))
            //The page object needs to present and exactly like how it is specified here. 20 is entered as the size
            // because that's the default in the configuration if no size parameter has been given
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntry(0, 20))))
            //The authors need to be embedded in the values, all 4 of them have to be present as the size allows it
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(FacetValueMatcher.entryAuthor("Doe, Jane"),
                FacetValueMatcher.entryAuthor("Smith, Maria"), FacetValueMatcher.entryAuthor("Doe, John"),
    ...
    public void discoverFacetsAuthorTestForPagination() throws Exception {
        ...
        Item publicItem2 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2016-02-13")
            ...
            .withSubject("TestingForMore").withSubject("ExtraEntry").build();
        ...
        getClient().perform(get("/api/discover/facets/author").param("size", "2").param("page", "1"))
            ...
            .andExpect(jsonPath("$.facetType", is("text")))
            //There needs to be a next link because there are more authors than the current size is allowed to show.
            // There are more pages after this one
            .andExpect(jsonPath("$._links.next.href", containsString("api/discover/facets/author?page")))
            //There always needs to be a self link
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/author")))
            //The page object has to be like this because that's what we've asked in the parameters
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntry(1, 2))))
            //These authors have to be present because of the current configuration
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(FacetValueMatcher.entryAuthor("Doe, John"),
                FacetValueMatcher.entryAuthor("Smith, Donald"))));
    }
    ...
    public void discoverFacetsTestWithSimpleQueryAndSearchFilter() throws Exception {
        ...
        //2. Three public items that are readable by Anonymous with different subjects
        Item publicItem1 = ItemBuilder.createItem(context, col1).withTitle("Test").withIssueDate("2017-10-17")
            .withAuthor("Smith, Donald").withAuthor("Testing, Works").withSubject("ExtraEntry").build();
        ...
        getClient().perform(get("/api/discover/facets/author").param("f.title", "test,contains"))
            ...
            //The applied filters have to be specified like this, applied filters are the parameters given below
            // starting with f.
            .andExpect(jsonPath("$.appliedFilters",
                contains(AppliedFilterMatcher.appliedFilterEntry("title", "contains", "test", "test"))))
            //This is how the page object must look like because it's the default
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntry(0, 20))))
            //These authors need to be present in the result because these have made items that contain 'test' in the
            // title
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(FacetValueMatcher.entryAuthor("Smith, Donald"),
            ...
            .andExpect(jsonPath("$._embedded.values",
                not(containsInAnyOrder(FacetValueMatcher.entryAuthor("Smith, Maria"),
    ...
    public void discoverFacetsDateTest() throws Exception {
        ...
        //2. Three public items that are readable by Anonymous with different subjects
        Item publicItem1 = ItemBuilder.createItem(context, col1).withTitle("Test").withIssueDate("2010-10-17")
            .withAuthor("Smith, Donald").withAuthor("Testing, Works").withSubject("ExtraEntry").build();

        Item publicItem2 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("1990-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("Testing, Works")
            .withSubject("TestingForMore").withSubject("ExtraEntry").build();

        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2000-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        //** WHEN **
        ...
        getClient().perform(get("/api/discover/facets/dateIssued"))
            ...
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(
                //We'll always get atleast two intervals with the items specified above, so we ask to match twice
                // atleast
                FacetValueMatcher.entryDateIssued(), FacetValueMatcher.entryDateIssued())));
    }
    ...
    public void discoverFacetsTestWithScope() throws Exception {
        ...
        getClient().perform(get("/api/discover/facets/author").param("scope", "testScope"))
            ...
            //There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/author")))
            //These are all the authors for the items that were created and thus they have to be present in the
            // embedded values section
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(FacetValueMatcher.entryAuthor("Doe, Jane"),
                FacetValueMatcher.entryAuthor("Smith, Maria"), FacetValueMatcher.entryAuthor("Doe, John"),
                FacetValueMatcher.entryAuthor("Smith, Donald"))));
        //** WHEN **
        ...
        //With a certain scope
        //And a size of 2
        getClient().perform(get("/api/discover/facets/author").param("scope", "testScope").param("size", "2"))
            //** THEN **
            //The status has to be 200 OK
            .andExpect(status().isOk())
            //The type has to be 'discover'
            ...
            //There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/author")))
            //These are the values that need to be present as it's ordered by count and these authors are the most
            // common ones in the items that we've created
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(FacetValueMatcher.entryAuthor("Doe, Jane"),
                FacetValueMatcher.entryAuthor("Smith, Maria"))));
    }
    ...
    public void discoverFacetsDateTestForHasMore() throws Exception {
        ...
        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("1940-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();

        Item publicItem4 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("1950-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();

        Item publicItem5 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("1960-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();

        Item publicItem6 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("1970-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();

        Item publicItem7 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("1980-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        ...
        //An anonymous user browses this endpoint to find the dateIssued results by the facet
        //And a size of 2
        getClient().perform(get("/api/discover/facets/dateIssued").param("size", "2"))
            ...
            //There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/dateIssued")))
            //Seeing as we've entered a size of two and there are more dates than just two, we'll need a next link to
            // go to the next page to see the rest of the dates
            .andExpect(jsonPath("$._links.next.href", containsString("api/discover/facets/dateIssued?page")))
            //The page object needs to look like this because we've entered a size of 2 and we didn't specify a
            ...
            .andExpect(jsonPath("$._embedded.values",
                containsInAnyOrder(FacetValueMatcher.entryDateIssued(), FacetValueMatcher.entryDateIssued())));
    }
    ...
    public void discoverFacetsDateTestWithSearchFilter() throws Exception {
        ...
        Item publicItem2 = ItemBuilder.createItem(context, col2).withTitle("Test 2").withIssueDate("1990-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("Testing, Works")
            .withSubject("TestingForMore").withSubject("ExtraEntry").build();

        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2010-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        //** WHEN **
        ...
        //And a size of 2
        getClient().perform(get("/api/discover/facets/dateIssued").param("f.title", "test,contains").param("size", "2"))
            ...
            //There always needs to be a self link
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/dateIssued")))
            //There needs to be an appliedFilters section that looks like this because we've specified a query in the
            // parameters
            .andExpect(jsonPath("$.appliedFilters",
                containsInAnyOrder(AppliedFilterMatcher.appliedFilterEntry("title", "contains", "test", "test"))))
            //The page object needs to look like this because we entered a size of 2 and we didn't specify a starting
            // page so it defaults to 0
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntry(0, 2))))
            //There needs to be only two date intervals with a count of 1 because of the query we specified
            .andExpect(jsonPath("$._embedded.values",
                containsInAnyOrder(FacetValueMatcher.entryDateIssuedWithCountOne(),
    ...
    public void discoverSearchTest() throws Exception {
        ...
        getClient().perform(get("/api/discover/search"))
            ...
            //There needs to be a section where these filters as specified as they're the default filters given in
            // the configuration
            .andExpect(jsonPath("$.filters",
                containsInAnyOrder(SearchFilterMatcher.titleFilter(), SearchFilterMatcher.authorFilter(),
                    SearchFilterMatcher.subjectFilter(), SearchFilterMatcher.dateIssuedFilter(),
            ...
            .andExpect(jsonPath("$.sortOptions",
                containsInAnyOrder(SortOptionMatcher.titleSortOption(), SortOptionMatcher.dateIssuedSortOption(),
    ...
    public void discoverSearchObjectsTest() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects"))
            ...
            //The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            //There needs to be a page object that shows the total pages and total elements as well as the size and
            // the current page (number)
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 7))))
            //These search results have to be shown in the embedded.objects section as these are the items given in
            // the structure defined above.
            //Seeing as everything fits onto one page, they have to all be present
            .andExpect(jsonPath("$._embedded.objects", Matchers
                .containsInAnyOrder(SearchResultMatcher.match("community", "communities"),
            ...
            .andExpect(jsonPath("$._embedded.facets", Matchers
                .containsInAnyOrder(FacetEntryMatcher.authorFacet(false), FacetEntryMatcher.subjectFacet(false),
    ...
    public void discoverSearchObjectsTestHasMoreAuthorFacet() throws Exception {
        ...
        //2. Three public items that are readable by Anonymous with different subjects
        Item publicItem1 = ItemBuilder.createItem(context, col1).withTitle("Test").withIssueDate("2010-10-17")
            .withAuthor("Smith, Donald").withAuthor("Testing, Works").withAuthor("a1, a1").withAuthor("b, b")
            .withAuthor("c, c").withAuthor("d, d").withAuthor("e, e").withAuthor("f, f").withAuthor("g, g")
            .withSubject("ExtraEntry").build();
        ...
        getClient().perform(get("/api/discover/search/objects"))
            ...
            //The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            //The page object has to look like this because we've only made 7 elements, the default size is 20 and
            // they all fit onto one page (20 > 7) so totalPages has to be 1. Number is 0 because
            //page 0 is the default page we view if not specified otherwise
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 7))))
            //All elements have to be present in the embedded.objects section, these are the ones we made in the
            ...
            .andExpect(jsonPath("$._embedded.facets", Matchers
                .containsInAnyOrder(FacetEntryMatcher.authorFacet(true), FacetEntryMatcher.subjectFacet(false),
                    FacetEntryMatcher.dateIssuedFacet(false),
                    FacetEntryMatcher.hasContentInOriginalBundleFacet(false))))
            //There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("/api/discover/search/objects")));
    ...
    public void discoverSearchObjectsTestHasMoreSubjectFacet() throws Exception {
        ...
        //2. Three public items that are readable by Anonymous with different subjects
        Item publicItem1 = ItemBuilder.createItem(context, col1).withTitle("Test").withIssueDate("2010-10-17")
            .withAuthor("Smith, Donald").withAuthor("Testing, Works").withSubject("ExtraEntry").withSubject("a")
            .withSubject("b").withSubject("c").withSubject("d").withSubject("e").withSubject("f").withSubject("g")
            .withSubject("h").withSubject("i").withSubject("j").withSubject("k").build();
        ...
        getClient().perform(get("/api/discover/search/objects"))
            ...
            //The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            //The page object has to look like this because we've only made 7 items, they all fit onto 1 page because
            // the default size is 20 and the default starting page is 0.
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 7))))
            ...
            .andExpect(jsonPath("$._embedded.facets", Matchers
                .containsInAnyOrder(FacetEntryMatcher.authorFacet(false), FacetEntryMatcher.subjectFacet(true),
                    FacetEntryMatcher.dateIssuedFacet(false),
                    FacetEntryMatcher.hasContentInOriginalBundleFacet(false))))
            //There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("/api/discover/search/objects")));
    ...
    public void discoverSearchObjectsTestWithBasicQuery() throws Exception {
        ...
        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2010-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").build();

        //** WHEN **
        //An anonymous user browses this endpoint to find the the objects in the system
        //With a query that says that the title has to contain 'test'
        getClient().perform(get("/api/discover/search/objects").param("f.title", "test,contains"))
            ...
            //The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            //The page object has to look like this because of the query we specified, only two elements match the
            // query.
            ...
            .andExpect(jsonPath("$._embedded.objects", Matchers
                .containsInAnyOrder(SearchResultMatcher.match("item", "items"),
                    SearchResultMatcher.match("item", "items"))))
    ...
    public void discoverSearchObjectsTestWithScope() throws Exception {
        ...
        //An anonymous user browses this endpoint to find the the objects in the system
        //With a scope 'test'
        getClient().perform(get("/api/discover/search/objects").param("scope", "test"))
    ...
    public void discoverSearchObjectsTestWithDsoType() throws Exception {
        ...
        //An anonymous user browses this endpoint to find the the objects in the system
        //With a dsoType 'item'
        getClient().perform(get("/api/discover/search/objects").param("dsoType", "item"))
            ...
            //The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            //The page element needs to look like this and only have three totalElements because we only want the
            // items (dsoType) and we only created three items
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 3))))
            //Only the three items can be present in the embedded.objects section as that's what we specified in the
            // dsoType parameter
            .andExpect(jsonPath("$._embedded.objects", Matchers
                .containsInAnyOrder(SearchResultMatcher.match("item", "items"),
                    SearchResultMatcher.match("item", "items"), SearchResultMatcher.match("item", "items"))))
    ...
    public void discoverSearchObjectsTestWithDsoTypeAndSort() throws Exception {
        ...
        Item publicItem2 = ItemBuilder.createItem(context, col2).withTitle("Testing").withIssueDate("1990-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("Testing, Works")
            .withSubject("TestingForMore").withSubject("ExtraEntry").build();

        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public").withIssueDate("2010-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        ...
        getClient().perform(get("/api/discover/search/objects").param("dsoType", "item").param("sort", "dc.title,ASC"))
            ...
            //The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            //The page object has to look like this and only contain three total elements because we only want to get
            // the items back
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 3))))
            //Only the three items can be present in the embedded.objects section as that's what we specified in the
            // dsoType parameter
            .andExpect(jsonPath("$._embedded.objects", Matchers
                .containsInAnyOrder(SearchResultMatcher.match("item", "items"),
                    SearchResultMatcher.match("item", "items"), SearchResultMatcher.match("item", "items"))))
            //Here we want to match on the item name in a certain specified order because we want to check the sort
            // properly
            //We check whether the items are sorted properly as we expected
            .andExpect(jsonPath("$._embedded.objects", Matchers
                .contains(SearchResultMatcher.matchOnItemName("item", "items", "Public"),
            ...
            .andExpect(jsonPath("$.sort", is(SortOptionMatcher.sortByAndOrder("dc.title", "ASC"))))
            //There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("/api/discover/search/objects")));
    }
    ...
    public void discoverFacetsDateTestWithLabels() throws Exception {
        ...
        Item publicItem6 = ItemBuilder.createItem(context, col2).withTitle("Public").withIssueDate("2017-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        Item publicItem7 = ItemBuilder.createItem(context, col2).withTitle("Public").withIssueDate("2010-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        Item publicItem8 = ItemBuilder.createItem(context, col2).withTitle("Public").withIssueDate("1990-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        Item publicItem9 = ItemBuilder.createItem(context, col2).withTitle("Public").withIssueDate("1970-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        Item publicItem10 = ItemBuilder.createItem(context, col2).withTitle("Public").withIssueDate("1950-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        Item publicItem11 = ItemBuilder.createItem(context, col2).withTitle("Public").withIssueDate("1930-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        Item publicItem12 = ItemBuilder.createItem(context, col2).withTitle("Public").withIssueDate("1910-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        Item publicItem13 = ItemBuilder.createItem(context, col2).withTitle("Public").withIssueDate("1890-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        Item publicItem14 = ItemBuilder.createItem(context, col2).withTitle("Public").withIssueDate("1866-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").build();
        ...
        getClient().perform(get("/api/discover/facets/dateIssued"))
            ...
            .andExpect(jsonPath("$._embedded.values", Matchers
                .containsInAnyOrder(FacetValueMatcher.entryDateIssuedWithLabel("2000 - 2017"),
    ...
    public void discoverSearchObjectsTestForPaginationAndNextLinks() throws Exception {
        ...
        //2. Three public items that are readable by Anonymous with different subjects
        Item publicItem1 = ItemBuilder.createItem(context, col1).withTitle("Test").withIssueDate("2010-10-17")
            .withAuthor("Smith, Donald").withAuthor("t, t").withAuthor("t, y").withAuthor("t, r").withAuthor("t, e")
            .withAuthor("t, z").withAuthor("t, a").withAuthor("t, tq").withAuthor("t, ts").withAuthor("t, td")
            .withAuthor("t, tf").withAuthor("t, tg").withAuthor("t, th").withAuthor("t, tj").withAuthor("t, tk")
            .withSubject("ExtraEntry").build();
        ...
        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2010-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").withSubject("a").withSubject("b").withSubject("c").withSubject("d")
            .withSubject("e").withSubject("f").withSubject("g").withSubject("h").withSubject("i").withSubject("j")
            .build();

        //** WHEN **
        //An anonymous user browses this endpoint to find the the objects in the system
        //With a size 2
        getClient().perform(get("/api/discover/search/objects").param("size", "2").param("page", "1"))
            //** THEN **
            //The status has to be 200 OK
            .andExpect(status().isOk())
            //The type has to be 'discover'
            ...
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntryWithTotalPagesAndElements(1, 2, 4, 7))))
            //These are the  two elements that'll be shown (because page = 1, so the third and fourth element in the
            // list) and they'll be the only ones because the size is 2
            .andExpect(jsonPath("$._embedded.objects",
                Matchers.containsInAnyOrder(SearchResultMatcher.match(), SearchResultMatcher.match()))).andExpect(
            jsonPath("$._embedded.facets", Matchers
                .containsInAnyOrder(FacetEntryMatcher.authorFacet(true), FacetEntryMatcher.subjectFacet(true),
                    FacetEntryMatcher.dateIssuedFacet(false),
                    FacetEntryMatcher.hasContentInOriginalBundleFacet(false))))
            //There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("/api/discover/search/objects")));
    ...
    public void discoverSearchObjectsTestWithContentInABitstream() throws Exception {
        ...
        try (InputStream is = IOUtils.toInputStream(bitstreamContent, CharEncoding.UTF_8)) {
            Bitstream bitstream = BitstreamBuilder.
                createBitstream(context, publicItem1, is).withName("Bitstream").withMimeType("text/plain").build();
        }
        ...
        getClient().perform(get("/api/discover/search/objects").param("query", "ThisIsSomeDummyText"))
            ...
            .andExpect(jsonPath("$._embedded.objects",
                Matchers.contains(SearchResultMatcher.matchOnItemName("item", "items", "Test"))))
    ...
    public void discoverSearchObjectsTestForEmbargoedItemsAndPrivateItems() throws Exception {
        ...
        Item publicItem2 = ItemBuilder.createItem(context, col2).withTitle("Test 2").withIssueDate("1990-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("Testing, Works")
            .withSubject("TestingForMore").withSubject("ExtraEntry").makePrivate().build();

        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2010-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").withEmbargoPeriod("12 months").build();
    ...
    public void discoverSearchObjectsTestWithContentInAPrivateBitstream() throws Exception {
        ...
        try (InputStream is = IOUtils.toInputStream(bitstreamContent, CharEncoding.UTF_8)) {
            Bitstream bitstream = BitstreamBuilder.
                createBitstream(context, publicItem1, is).withName("Bitstream")
                .withDescription("Test Private Bitstream").withMimeType("text/plain").withReaderGroup(internalGroup)
                .build();
        }
        ...
        getClient().perform(get("/api/discover/search/objects").param("query", "ThisIsSomeDummyText"))
            ...
            .andExpect(jsonPath("$._embedded.object",
                Matchers.not(Matchers.contains(SearchResultMatcher.matchOnItemName("item", "items", "Test")))))
    ...
    public void discoverSearchObjectsTestForScope() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects").param("scope", String.valueOf(scope)))
            //** THEN **
            //The status has to be 200 OK
            .andExpect(status().isOk())
            ...
            .andExpect(jsonPath("$._embedded.objects", Matchers
                .containsInAnyOrder(SearchResultMatcher.matchOnItemName("item", "items", "Test 2"),
                    SearchResultMatcher.matchOnItemName("item", "items", "Public item 2"))))
            //These facets have to show up in the embedded.facets section as well with the given hasMore property
            // because we don't exceed their default limit for a hasMore true (the default is 10)
            .andExpect(jsonPath("$._embedded.facets", Matchers
    ...
    public void discoverSearchObjectsTestForScopeWithPrivateItem() throws Exception {
        ...
        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2010-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withAuthor("test,test").withAuthor("test2, test2")
            .withAuthor("Maybe, Maybe").withSubject("AnotherTest").withSubject("TestingForMore")
            .withSubject("ExtraEntry").makePrivate().build();
        ...
        getClient().perform(get("/api/discover/search/objects").param("scope", String.valueOf(scope)))
            ...
            .andExpect(jsonPath("$._embedded.objects",
                Matchers.contains(SearchResultMatcher.matchOnItemName("item", "items", "Test 2")
                    //                        SearchResultMatcher.matchOnItemName("item", "items", "Public item 2")
                )))
            //Make sure that the search result doesn't contain the item that's set to private but does have the
            // correct scope
            .andExpect(jsonPath("$._embedded.objects",
                Matchers.not(Matchers.contains(SearchResultMatcher.matchOnItemName("item", "items", "Public item 2")))))
            //These facets have to show up in the embedded.facets section as well with the given hasMore property
            // because we don't exceed their default limit for a hasMore true (the default is 10)
            .andExpect(jsonPath("$._embedded.facets", Matchers
                .containsInAnyOrder(FacetEntryMatcher.authorFacet(false), FacetEntryMatcher.subjectFacet(false),
    ...
    public void discoverSearchObjectsTestForHitHighlights() throws Exception {
        ...
        Item publicItem2 = ItemBuilder.createItem(context, col2).withTitle("Test 2").withIssueDate("1990-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withSubject("ExtraEntry").build();

        Item publicItem3 = ItemBuilder.createItem(context, col2).withTitle("Public item 2").withIssueDate("2010-02-13")
            .withAuthor("Smith, Maria").withAuthor("Doe, Jane").withSubject("AnotherTest").withSubject("ExtraEntry")
            .build();
        ...
        getClient().perform(get("/api/discover/search/objects").param("query", query))
            //** THEN **
            //The status has to be 200 OK
            .andExpect(status().isOk())
            //The type has to be 'discover'
            ...
            //The page object needs to look like this
            .andExpect(jsonPath("$.page", is(PageMatcher.pageEntry(0, 20))))
            //The search results has to contain the item with the query in the title and the hithighlight has to be
            // filled in with a string containing the query
            .andExpect(jsonPath("$._embedded.objects", Matchers.contains(SearchResultMatcher
    ...
    public void discoverSearchObjectsTestForHitHighlightsWithPrivateItem() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects").param("query", query))
            ...
            .andExpect(jsonPath("$._embedded.objects", Matchers.not(Matchers.contains(SearchResultMatcher
                .matchOnItemNameAndHitHighlight("item", "items", "Public item 2", query, "dc.title")))))
            //There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("/api/discover/search/objects")));
