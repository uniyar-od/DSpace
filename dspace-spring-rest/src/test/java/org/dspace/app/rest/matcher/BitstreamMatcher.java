...
public class BitstreamMatcher {
    ...
    public static Matcher<? super Object> matchBitstreamEntry(Bitstream bitstream) {
        return allOf(
            ...
            hasJsonPath("$.uuid", is(bitstream.getID().toString())), hasJsonPath("$.name", is(bitstream.getName())),
    ...
    public static Matcher<? super Object> matchBitstreamEntry(UUID uuid, long size) {
        return allOf(
            //Check ID and size
            hasJsonPath("$.uuid", is(uuid.toString())), hasJsonPath("$.sizeBytes", is((int) size)),
    ...
    private static Matcher<? super Object> matchBitstreamLinks(UUID uuid) {
        return allOf(hasJsonPath("$._links.format.href", containsString("/api/core/bitstreams/" + uuid + "/format")),
    ...
    private static Matcher<? super Object> matchChecksum() {
        return allOf(hasJsonPath("$.checkSumAlgorithm", not(empty())), hasJsonPath("$.value", not(empty())));
    }

    private static Matcher<? super Object> matchFormat() {
        return allOf(hasJsonPath("$.mimetype", not(empty())), hasJsonPath("$.type", is("bitstreamformat")),
