                ...
                if (links != null && rels != null) {
                    ...
                    for (LinkRest linkAnnotation : links.links()) {
                        ...
                        LinkRestRepository linkRepository =
                            utils.getLinkResourceRepository(apiCategory, model, linkAnnotation.name());
                        ...
                        try {
                            ...
                            if (!found) {
                                throw new RuntimeException(
                                    "Method for relation " + linkAnnotation.name() + " not found: " +
                                    linkAnnotation.method());
                            }
                            ...
                            if (StringUtils.isBlank(linkAnnotation.method())) {
                                ...
                                if (linkedObject instanceof RestAddressableModel) {
                                    ...
                                    if (linkedObject instanceof List) {
                                        List<RestAddressableModel> linkedRMList =
                                    ...
                                    for (Method m : methods) {
                                        if (StringUtils.equals(m.getName(), linkAnnotation.method())) {
                                            if (Page.class.isAssignableFrom(m.getReturnType())) {
                                                Page<? extends Serializable> pageResult =
                                                    (Page<? extends RestAddressableModel>) m
                                                        .invoke(linkRepository, null, ((BaseObjectRest) data).getId(),
                                                            null, null);
                                                EmbeddedPage ep =
                                                    new EmbeddedPage(linkToSubResource.getHref(), pageResult, null);
                                                embedded.put(name, ep);
                                            } else {
                                                RestAddressableModel object = (RestAddressableModel) m
                                                    .invoke(linkRepository, null, ((BaseObjectRest) data).getId(), null,
                                                        null);
                                                HALResource ep =
                                                    linkRepository.wrapResource(object, linkToSubResource.getHref());
                                                embedded.put(name, ep);
                                            }
