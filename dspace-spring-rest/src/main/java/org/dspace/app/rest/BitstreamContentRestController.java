...
public class BitstreamContentRestController implements InitializingBean {
    ...
    public void retrieve(@PathVariable UUID uuid, HttpServletResponse response, HttpServletRequest request)
        ...
        try (InputStream is = bitstreamTuple.getLeft()) {
            MultipartFileSender sender =
                MultipartFileSender.fromInputStream(is).withBufferSize(BUFFER_SIZE).withFileName(name)
                    .withLength(bitstreamTuple.getRight()).withChecksum(bit.getChecksum()).withMimetype(mimetype)
                    .withLastModified(lastModified).with(request).with(response);
            ...
            log.debug(
                "Client aborted the request before the download was completed. Client is probably switching to a " +
                "Range request.",
                ex);
        }
    }
