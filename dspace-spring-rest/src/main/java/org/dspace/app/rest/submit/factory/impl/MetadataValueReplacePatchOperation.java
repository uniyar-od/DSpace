...
public abstract class MetadataValueReplacePatchOperation<DSO extends DSpaceObject>
    ...
    protected void replaceValue(Context context, DSO source, String target, List<IMetadataValue> list,
                                MetadataValueRest object, int index) throws SQLException {
        String[] metadata = Utils.tokenize(target);
        getDSpaceObjectService()
            .replaceMetadata(context, source, metadata[0], metadata[1], metadata[2], object.getLanguage(),
    ...
    protected void setDeclaredField(Context context, DSO source, Object value, String metadata, String namedField,
                                    List<IMetadataValue> metadataByMetadataString, int index)
