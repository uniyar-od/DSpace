    ...
    public SubmissionDefinitionRest fromModel(SubmissionConfig obj) {
        ...
        try {
            ...
            List<Collection> collections = panelConverter.getSubmissionConfigReader()
                .getCollectionsBySubmissionConfig(context, obj.getSubmissionName());
            List<CollectionRest> collectionsRest =
                collections.stream().map((collection) -> collectionConverter.convert(collection))
