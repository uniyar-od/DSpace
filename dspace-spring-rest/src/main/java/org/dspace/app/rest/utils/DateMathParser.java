...
public class DateMathParser {
    ...
    public static final DateTimeFormatter PARSER =
        new DateTimeFormatterBuilder().parseCaseInsensitive().parseLenient().appendInstant().toFormatter(Locale.ROOT);
    ...
    public static Date parseMath(Date now, String val) throws Exception {
        ...
        if (val.startsWith("NOW")) {
            ...
            if (zz == -1) {
                throw new Exception(BAD_REQUEST + "Invalid Date String:'" + val + '\'');
            }
            math = val.substring(zz + 1);
            try {
                p.setNow(parseNoMath(val.substring(0, zz + 1)));
            } catch (DateTimeParseException e) {
                throw new Exception(BAD_REQUEST + "Invalid Date in Date Math String:'" + val + '\'', e);
            }
        }
        ...
        try {
            ...
            throw new Exception(BAD_REQUEST + "Invalid Date Math String:'" + val + '\'', e);
        }
    }
