...
public abstract class MetadataValueAddPatchOperation<DSO extends DSpaceObject>
    ...
    protected void replaceValue(Context context, DSO source, String target, List<MetadataValueRest> list)
        ...
        for (MetadataValueRest ll : list) {
            getDSpaceObjectService()
                .addMetadata(context, source, metadata[0], metadata[1], metadata[2], ll.getLanguage(), ll.getValue(),
                    ll.getAuthority(), ll.getConfidence());
        }
    }
    ...
    protected void addValue(Context context, DSO source, String target, MetadataValueRest object, int index)
        ...
        if (index == -1) {
            getDSpaceObjectService()
                .addMetadata(context, source, metadata[0], metadata[1], metadata[2], object.getLanguage(),
                    object.getValue(), object.getAuthority(), object.getConfidence());
        } else {
            getDSpaceObjectService()
                .addAndShiftRightMetadata(context, source, metadata[0], metadata[1], metadata[2], object.getLanguage(),
                    object.getValue(), object.getAuthority(), object.getConfidence(), index);
        }
    }
