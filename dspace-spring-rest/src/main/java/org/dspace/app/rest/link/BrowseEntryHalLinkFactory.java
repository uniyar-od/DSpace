...
public class BrowseEntryHalLinkFactory extends HalLinkFactory<BrowseEntryResource, RestResourceController> {
    ...
    protected void addLinks(final BrowseEntryResource halResource, final Pageable pageable, final LinkedList<Link> list)
        ...
        if (data != null) {
            ...
            UriComponentsBuilder baseLink = uriBuilder(getMethodOn(bix.getCategory(), bix.getType())
                .findRel(null, bix.getCategory(), English.plural(bix.getType()), bix.getId(), BrowseIndexRest.ITEMS,
                    null, null, null));

            addFilterParams(baseLink, data);

            list.add(buildLink(BrowseIndexRest.ITEMS, baseLink.build().toUriString()));
        }
    }
