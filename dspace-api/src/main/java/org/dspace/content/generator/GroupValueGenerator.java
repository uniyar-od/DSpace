    ...
    public List<IMetadataValue> generator(Context context, Item targetItem, Item templateItem,
        ...
        try {
            if (StringUtils.startsWith(prefix, "community")) {
                ...
                Collection collection = (Collection) templateItem.getItemService()
                                                                 .getParentObject(context, templateItem);
                value = ((Community) collection.getDSpaceObjectService().getParentObject(context, collection))
