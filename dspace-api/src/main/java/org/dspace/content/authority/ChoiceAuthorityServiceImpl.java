    ...
    private Map<String, ChoiceAuthority> getChoiceAuthorityMap() {
        ...
        if (controller.isEmpty()) {
            ...
            try {
                ...
                String defaultfKey = configurationService
                    .getProperty(CHOICES_PLUGIN_PREFIX + MetadataSchema.DC_SCHEMA + ".authority.default");
                ...
                List<MetadataField> tmp = metadataFieldService
                    .findFieldsByElementNameUnqualified(context, MetadataSchema.DC_SCHEMA, "authority");
                for (MetadataField mf : tmp) {
                    String tmpKey = makeFieldKey(MetadataSchema.DC_SCHEMA, mf.getElement(), mf.getQualifier());
                    String tmpfKey = configurationService.getProperty(
                        CHOICES_PLUGIN_PREFIX + MetadataSchema.DC_SCHEMA + "." + mf.getElement() + "." + mf
                            .getQualifier());
            ...
            while (keyIterator.hasNext()) {
                ...
                if (ma == null) {
                    log.warn(
                        "Skipping invalid configuration for " + key + " because named plugin not found: " +
