...
public class ItemAuthorityConsumer implements Consumer {
            ...
            for (IMetadataValue md : meta) {
                if (md.getAuthority() != null) {
                    DSpaceObject dso = HandleServiceFactory.getInstance().getHandleService()
                                                           .resolveToObject(ctx, md.getAuthority());
                    if (dso != null && dso instanceof Item) {
                        Item target = (Item) dso;
                        needCommit = needCommit || assureReciprocalLink(ctx, target, reciprocalMetadata.get(m),
    ...
    private boolean assureReciprocalLink(Context ctx, Item target, String mdString, String name, String handle)
        throws SQLException {
        List<IMetadataValue> meta = target.getItemService().getMetadataByMetadataString(target, mdString);
        String[] mdSplit = mdString.split("\\.");
        target.getItemService()
              .clearMetadata(ctx, target, mdSplit[0], mdSplit[1], mdSplit.length > 2 ? mdSplit[2] : null, Item.ANY);
        boolean added = false;
        if (meta != null) {
            for (IMetadataValue md : meta) {
                ...
                target.getItemService()
                      .addMetadata(ctx, target, mdSplit[0], mdSplit[1], mdSplit.length > 2 ? mdSplit[2] : null, null,
                                   md.getValue(), md.getAuthority(), md.getConfidence());
            }
        }
        if (!added) {
            target.getItemService()
                  .addMetadata(ctx, target, mdSplit[0], mdSplit[1], mdSplit.length > 2 ? mdSplit[2] : null, null, name,
                               handle, Choices.CF_ACCEPTED);
        }
        return true;
    }
