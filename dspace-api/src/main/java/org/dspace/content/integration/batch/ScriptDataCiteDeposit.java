                ...
                if ("oracle".equals(dbName)) {
                    rows = getHibernateSession(context).createSQLQuery(
                        "select d2i.item_id as item_id, criteria, identifier_doi from "
                            + TABLE_NAME_DOI2ITEM
                            + " d2i left join item i on d2i.item_id = i.uuid where (d2i.last_modified is null OR d2i" +
                            ".last_modified < i.last_modified)"
                            + " AND ROWNUM <= " + limit).addScalar("item_id", UUIDCharType.INSTANCE)
                                                       .addScalar("criteria").addScalar("identifier_doi").list();
                } else {
                    rows = getHibernateSession(context).createSQLQuery(
                        "select d2i.item_id, criteria, identifier_doi from "
                            ...
                            + " d2i left join item i on d2i.item_id = i.uuid where (d2i.last_modified is null OR d2i" +
                            ".last_modified < i.last_modified)"
                            + " LIMIT " + limit).addScalar("item_id", UUIDCharType.INSTANCE).addScalar("criteria")
                                                       .addScalar("identifier_doi").list();
                }
                int offset = 0;
                int count = 0;
                        ...
                        if ("oracle".equals(dbName)) {
                            rows = getHibernateSession(context).createSQLQuery(
                                "select d2i.item_id, criteria, identifier_doi from "
                                    ...
                                    + limit
                                    + " AND ROWNUM <= "
                                    + (offset + limit)).addScalar("item_id", UUIDCharType.INSTANCE)
                                                               .addScalar("criteria").addScalar("identifier_doi")
                                                               .list();
                        } else {
                            rows = getHibernateSession(context).createSQLQuery(
                                "select d2i.item_id, criteria, identifier_doi from "
                                    ...
                                    + " d2i left join item i on d2i.item_id = i.uuid where d2i.last_modified is null"
                                    + " LIMIT " + limit
                                    + " OFFSET " + offset).addScalar("item_id", UUIDCharType.INSTANCE)
                                                               .addScalar("criteria").addScalar("identifier_doi")
                                                               .list();
                        }
                    }
                ...
                if (line.hasOption('s')) {
                    ...
                    List<Object[]> rows = getHibernateSession(context).createSQLQuery(
                        "SELECT item_id, criteria, identifier_doi FROM "
                            + TABLE_NAME_DOI2ITEM
                            + " d2i left join item i on d2i.item_id = i.uuid where d2i.item_id = :item_id AND d2i" +
                            ".last_modified is null")
                                                                      .addScalar("item_id", UUIDCharType.INSTANCE)
                                                                      .addScalar("criteria").addScalar("identifier_doi")
                                                                      .setParameter("item_id",
                                                                                    id).list();
        ...
        try {
            ...
            if (responseCode == 201) {
                ...
                getHibernateSession(context).createSQLQuery(
                    "UPDATE "
                        + TABLE_NAME_DOI2ITEM
                        + " SET LAST_MODIFIED = :last_modified, RESPONSE_CODE = :response_code, NOTE = :note, " +
                        "FILENAME = :filename WHERE ITEM_ID = :item_id")
                                            .setParameter("last_modified",
                                                          new java.sql.Timestamp(new Date().getTime()))
                                            .setParameter("response_code", responseCode).setParameter("note",
                                                                                                      result.get(target
                                                                                                                     .getID()))
                                            .setParameter("filename", urlDeposited)
                                            .setParameter("item_id", target.getID()).executeUpdate();
            } else {
                getHibernateSession(context).createSQLQuery("UPDATE "
                                                                + TABLE_NAME_DOI2ITEM
                                                                + " SET RESPONSE_CODE = :response_code, NOTE = :note " +
                                                                "WHERE ITEM_ID = :item_id")
                                            .setParameter("response_code", responseCode)
