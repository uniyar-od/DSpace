            ...
            if (line.hasOption('a')) {
                ...
                while (!rows.isEmpty()) {
                        ...
                        if ("oracle".equals(dbName)) {
                            rows = getHibernateSession(context).createSQLQuery(
                                "select d2i.item_id, criteria, identifier_doi from "
                                    ...
                                    "OR d2i.last_modified < i.last_modified"
                                    + " AND ROWNUM > " + limit
                    ...
                    for (Object[] row : rows) {
                        ...
                        Item item = ContentServiceFactory.getInstance().getItemService()
                                                         .find(context, UUID.fromString((String) row[0]));
                ...
                if (line.hasOption('s')) {
                    ...
                    List<Object[]> rows = getHibernateSession(context).createSQLQuery(
                        "SELECT d2i.item_id, criteria, identifier_doi FROM "
                            + TABLE_NAME_DOI2ITEM
                            + " d2i left join item i on d2i.item_id = i.item_id where d2i.item_id = :item_id AND (d2i" +
                            ".last_modified is null OR d2i.last_modified < i.last_modified)")
                                                                      .addScalar("item_id").addScalar("criteria")
                                                                      .addScalar("identifier_doi")
                                                                      .setParameter("item_id", id).list();
        ...
        try {
            ...
            if (responseCode == 200) {
                    ...
                    "UPDATE "
                        ...
                        "FILENAME = :filename WHERE ITEM_ID = :item_id")
                                            .setParameter("last_modified",
                                                          new Date()).setParameter("response_code", responseCode)
                                            .setParameter("note", result.get(target.getID()))
                                            .setParameter("filename", targetFile.getName())
                ...
                getHibernateSession(context).createSQLQuery("UPDATE "
                                            ...
                                            .setParameter("response_code",
                                                          responseCode).setParameter("note", result.get(target.getID()))
