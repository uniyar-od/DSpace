...
public class LDNEmailAction extends LDNAction {
	/*
	 * supported for actionSendFilter are: - <single email> - GROUP:<group_name> -
	 * SUBMITTER
	 */
	...
	public List<String> retrieveRecipientsEmail(Item item) throws SQLException {
		...
		if (actionSendFilter.startsWith("SUBMITTER")) {
			...
			String[] groupEmails = DSpaceServicesFactory.getInstance().getConfigurationService()
					.getArrayProperty("email." + groupName + ".list");
