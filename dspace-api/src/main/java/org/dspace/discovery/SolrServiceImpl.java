...
public class SolrServiceImpl implements SearchService, IndexingService {
    ...
    protected SolrServer getSolr() {
        if (solr == null) {
            String solrService =
                DSpaceServicesFactory.getInstance().getConfigurationService().getProperty("discovery.search.server");

            UrlValidator urlValidator = new UrlValidator(UrlValidator.ALLOW_LOCAL_URLS);
            if (urlValidator.isValid(solrService) ||
                ConfigurationManager.getBooleanProperty("discovery", "solr.url.validation.enabled", true)) {
                try {
                    ...
                    SolrQuery solrQuery =
                        new SolrQuery().setQuery(RESOURCE_TYPE_FIELD + ":2 AND " + RESOURCE_ID_FIELD + ":1");
     ...
     */
    @Override
    public void indexContent(Context context, BrowsableDSpaceObject dso) throws SQLException {
     ...
     */
    @Override
    public void indexContent(Context context, BrowsableDSpaceObject dso, boolean force) throws SQLException {
     ...
     */
    @Override
    public void unIndexContent(Context context, BrowsableDSpaceObject dso) throws SQLException, IOException {
     ...
     */
    @Override
    public void reIndexContent(Context context, BrowsableDSpaceObject dso) throws SQLException, IOException {
        try {
    ...
    protected void buildDocument(Context context, Community community) throws SQLException, IOException {
        ...
        // Create Document
        SolrInputDocument doc = buildDocument(Constants.COMMUNITY, community.getID(), community.getHandle(), locations);

        DiscoveryConfiguration discoveryConfiguration = SearchUtils.getDiscoveryConfiguration(community);
        DiscoveryHitHighlightingConfiguration highlightingConfiguration =
            discoveryConfiguration.getHitHighlightingConfiguration();
        ...
        List<SolrServiceIndexPlugin> solrServiceIndexPlugins =
            DSpaceServicesFactory.getInstance().getServiceManager().getServicesByType(SolrServiceIndexPlugin.class);
    ...
    protected void buildDocument(Context context, Collection collection) throws SQLException, IOException {
        ...
        SolrInputDocument doc =
            buildDocument(Constants.COLLECTION, collection.getID(), collection.getHandle(), locations);
    ...
    protected void addDiscoveryFields(SolrInputDocument doc, Context context, Item item,
        ...
        try {
            ...
            Map<String, DiscoverySortFieldConfiguration> sortFields =
                new HashMap<String, DiscoverySortFieldConfiguration>();
            Map<String, DiscoveryRecentSubmissionsConfiguration> recentSubmissionsConfigurationMap =
                new HashMap<String, DiscoveryRecentSubmissionsConfiguration>();
            ...
            for (DiscoveryConfiguration discoveryConfiguration : discoveryConfigurations) {
                ...
                if (hitHighlightingConfiguration != null) {
                    List<DiscoveryHitHighlightFieldConfiguration> fieldConfigurations =
                        hitHighlightingConfiguration.getMetadataFields();
                ...
                DiscoveryMoreLikeThisConfiguration moreLikeThisConfiguration =
                    discoveryConfiguration.getMoreLikeThisConfiguration();
            ...
            for (IMetadataValue meta : mydc) {
                ...
                String converterName = ConfigurationManager.getProperty("discovery", "converter." + field);
                if (converterName != null) {
                    IConverter iconverter = (IConverter) CoreServiceFactory.getInstance().getPluginService()
                                                                           .getNamedPlugin(IConverter.class,
                                                                                           converterName);
                ...
                if ((searchFilters.get(field) != null || searchFilters
                    ...
                    for (DiscoverySearchFilter searchFilter : searchFilterConfigs) {
                        ...
                        if (preferedLabel != null) {
                            ...
                            doc.addField(searchFilter.getIndexFieldName() + "_authority", authority);
                            doc.addField(searchFilter.getIndexFieldName() + "_acid",
                                         preferedLabel.toLowerCase() + separator + preferedLabel + AUTHORITY_SEPARATOR +
                                         authority);
                        }
                        if (variants != null) {
                ...
                if (toProjectionFields.contains(field) ||
                    ...
                    doc.addField(field + "_stored", value + STORE_SEPARATOR + preferedLabel + STORE_SEPARATOR +
                                                    (variantsToStore.length() >
                                                     VARIANTS_STORE_SEPARATOR.length() ? variantsToStore
                                                        .substring(VARIANTS_STORE_SEPARATOR.length()) : "null") +
                                                    STORE_SEPARATOR + authority + STORE_SEPARATOR + meta.getLanguage());
                }
    ...
    private void indexInProgressSubmissionItem(Context context, Item item)
        ...
        if (workflowItem == null) {
            ...
            List<DiscoveryConfiguration> discoveryConfigurations =
                SearchUtils.getAllDiscoveryConfigurations(workspaceItem);
            addDiscoveryFields(doc, context, item, discoveryConfigurations);
            ...
            List<DiscoveryConfiguration> discoveryConfigurations =
                SearchUtils.getAllDiscoveryConfigurations(workflowItem);
            addDiscoveryFields(doc, context, item, discoveryConfigurations);
    ...
    @Override
    public DiscoverResult search(Context context, BrowsableDSpaceObject dso, DiscoverQuery query)
    ...
    protected SolrQuery resolveToSolrQuery(Context context, DiscoverQuery discoveryQuery,
        ...
        List<SolrServiceSearchPlugin> solrServiceSearchPlugins =
            DSpaceServicesFactory.getInstance().getServiceManager().getServicesByType(SolrServiceSearchPlugin.class);
    ...
    @Override
    public InputStream searchJSON(Context context, DiscoverQuery query, BrowsableDSpaceObject dso,
                                  String jsonIdentifier) throws SearchServiceException {
        if (dso != null) {
            if (dso instanceof Community) {
    ...
    protected DiscoverResult retrieveResult(Context context, DiscoverQuery query, QueryResponse solrQueryResponse)
        ...
        if (solrQueryResponse != null) {
            ...
            for (SolrDocument doc : results) {
                ...
                if (solrQueryResponse.getHighlighting() != null) {
                    Map<String, List<String>> highlightedFields =
                        solrQueryResponse.getHighlighting().get(dso.getType() + "-" + dso.getID());
                ...
                for (int i = 0; i < facetFields.size(); i++) {
                    ...
                    if (facetValues != null) {
                        ...
                        for (FacetField.Count facetValue : facetValues) {
                            ...
                            DiscoverResult.FacetResult facetResult =
                                getDiscoveryFacet(context, facetField, facetValue, facetFieldConfig.getType());
            ...
            if (solrQueryResponse.getFacetQuery() != null) {
                ...
                LinkedHashMap<String, Integer> sortedFacetQueries =
                    new LinkedHashMap<String, Integer>(solrQueryResponse.getFacetQuery());
                for (String facetQuery : sortedFacetQueries.keySet()) {
                    ...
                    if (facetField == null) {
                        facetField = facetQuery.substring(0, facetQuery.indexOf(":"));
                        ...
                        result.addFacetResult(facetField,
                                              new DiscoverResult.FacetResult(filter, name, null, name, count,
                                                                             DiscoveryConfigurationParameters
                                                                                 .TYPE_DATE));
                        result.addFacetQueryResult(name,
                                                   new DiscoverResult.FacetResult(facetField, name, null, name, count,
                                                                                  DiscoveryConfigurationParameters
                                                                                      .TYPE_DATE));
                    }
                }
    ...
    public DiscoverResult.FacetResult getDiscoveryFacet(Context context, FacetField facetField,
                                                        FacetField.Count facetValue, String facetType)
        throws SQLException {
        ...
        DiscoverResult.FacetResult facetResult =
            new DiscoverResult.FacetResult(filterValue, displayedValue, authorityValue, sortValue,
                                           facetValue.getCount(), facetType);
        return facetResult;
    }
    ...
    public java.io.InputStream searchAsInputStream(DiscoverQuery query)
        ...
        HttpHost hostURL = (HttpHost) (((HttpSolrServer) getSolr()).getHttpClient().getParams()
        ...
        HttpResponse response = ((HttpSolrServer) getSolr()).getHttpClient().execute(method);
    ...
    public List<Item> getRelatedItems(Context context, Item item, DiscoveryMoreLikeThisConfiguration mltConfig) {
        ...
        try {
            ...
            @SuppressWarnings("unchecked") java.util.Collection<String> similarityMetadataFields =
                CollectionUtils.collect(mltConfig.getSimilarityMetadataFields(), new Transformer() {
                    @Override
    ...
    @Override
    public void indexContent(Context context, BrowsableDSpaceObject dso, boolean force, boolean commit)
