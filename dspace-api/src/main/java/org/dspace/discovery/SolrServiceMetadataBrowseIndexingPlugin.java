...
public class SolrServiceMetadataBrowseIndexingPlugin implements SolrServiceIndexPlugin {
    ...
    private String[] supportedLocales = StringUtils
        .split(ConfigurationManager.getProperty("webui.supported.locales"), ",");
            ...
            if (bi.isMetadataIndex()) {
                ...
                for (String lang : langToIndex) {
                    ...
                    if (item.isArchived() || item.isWithdrawn()) {
                        ...
                        for (int mdIdx = 0; mdIdx < bi.getMetadataCount(); mdIdx++) {
                            ...
                            if (values != null && values.size() > 0) {
                                int minConfidence = metadataAuthorityService.getMinConfidence(
                                    values.get(0).getMetadataField());
                                ...
                                for (int x = 0; x < values.size(); x++) {
                                    ...
                                    if (StringUtils.isEmpty(values.get(x).getValue())) {
                                        ...
                                        String language = lang != null ? StringUtils.trim(lang) : values.get(x)
                                                                                                        .getLanguage();
                                            ...
                                            boolean ignorePrefered =
                                                DSpaceServicesFactory.getInstance().getConfigurationService()
                                                    .getPropertyAsType(
                                                        ...
                                                        bi.getName(),
                                                        DSpaceServicesFactory.getInstance().getConfigurationService()
                                                            .getPropertyAsType("discovery" +
                                                                               ".browse" + ".authority" +
                                                                               ".ignore-prefered", Boolean.FALSE),
                                                        true);
                                            ...
                                            boolean ignoreVariants =
                                                DSpaceServicesFactory.getInstance().getConfigurationService()
                                                    .getPropertyAsType(
                                                        ...
                                                        bi.getName(),
                                                        DSpaceServicesFactory.getInstance().getConfigurationService()
                                                            .getPropertyAsType("discovery" + ".browse" + ".authority" +
                                                                               ".ignore-variants", new Boolean(false)),
                                                        true);
                                            if (!ignoreVariants) {
                    ...
                    for (String facet : distFValues) {
                        document.addField(distinctTableName + "_filter", facet);
                    }
                    for (String facet : distFAuths) {
                        document.addField(distinctTableName
                                          + "_authority_filter", facet);
                    }
                    for (String facet : distValuesForAC) {
                        document.addField(distinctTableName + "_partial", facet);
                    }
                    for (String facet : distFVal) {
                        document.addField(distinctTableName + "_value_filter", facet);
                    }
                }
            }
        }
