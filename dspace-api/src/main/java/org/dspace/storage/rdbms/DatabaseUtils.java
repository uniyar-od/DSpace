...
public class DatabaseUtils {
    ...
    private static final String reindexDiscoveryFilePath =
        DSpaceServicesFactory.getInstance().getConfigurationService().getProperty("dspace.dir") + File.separator +
        "solr" + File.separator + "search" + File.separator + "conf" + File.separator + "reindex.flag";

    private static final String rebuildCrisConfigFilePath =
        DSpaceServicesFactory.getInstance().getConfigurationService().getProperty("dspace.dir") + File.separator +
        "etc" + File.separator + "rebuildcrisconfiguration.flag";
    ...
    public static void main(String[] argv) {
        ...
        if (argv.length < 1) {
            ...
            System.out.println(
                "Valid actions: 'test', 'info', 'migrate', 'repair', 'validate', 'clean', 'install-example-cris-base'");
    ...
    private synchronized static Flyway setupFlyway(DataSource datasource) {
        ...
        if (flywaydb == null) {
            try (Connection connection = datasource.getConnection()) {
                ...
                List<String> workflowFlywayMigrationLocations =
                    WorkflowServiceFactory.getInstance().getWorkflowService().getFlywayMigrationLocations();
                ...
                List<FlywayCallback> flywayCallbacks =
                    DSpaceServicesFactory.getInstance().getServiceManager().getServicesByType(FlywayCallback.class);
        ...
        try {
            ...
            if (pending != null && pending.length > 0) {
                ...
                for (MigrationInfo info : pending) {
                    log.info("\t" + info.getVersion() + " " + info.getDescription() + " " + info.getType() + " " +
                             info.getState());
                }
    ...
    private static String determineDBVersion(Connection connection) throws SQLException {
        ...
        // First, is this a migration from DSpace to DSpace-CRIS? If the DSpace was already migrated with flyway
        // check for an "cris_rpage" table.
        ...
        if (tableExists(connection, "jdyna_widget_boolean")) {
            ...
            //WARNING!!! DOUBLE CHECK BECAUSE JDYNA_WIDGET_BOOLEAN COMES ALSO WITH DSPACECRIS_4.3.1 (no flyway in
            // version prior the 5_x_x)
            ...
            // Is this pre-DSpace 5.0 (with Helpdesk plugin)? Look for the "request_message" column in the
            // "requestitem" table
    ...
    private static String checkOldVersion(Connection connection, boolean check18) throws SQLException {
        if (check18) {
            // Is this DSpace 1.8.x? Look for the "bitstream_order" column in the "bundle2bitstream" table
            if (tableColumnExists(connection, "bundle2bitstream", "bitstream_order", null, null)) {
                return "1.8";
        ...
        throw new SQLException(
            "CANNOT AUTOUPGRADE DSPACE DATABASE, AS IT DOES NOT LOOK TO BE A VALID DSPACE DATABASE" + ".");
    }
     ...
     */
    public static boolean tableColumnExists(Connection connection, String tableName, String columnName, Integer type,
                                            Integer length) {
        boolean exists = false;
        ResultSet results = null;
    ...
    public static synchronized void setRebuildCrisConfiguration(boolean reindex) {
        ...
        if (reindex) {
            try {
                ...
                log.error("Unable to create Cris Configuration rebuild flag file " + reindexFlag.getAbsolutePath() +
                          ". You may need to rebuild manually.", io);
            }
        } else // Otherwise, CRIS configuration doesn't need to reindex. Delete the temporary file if it exists
        {
            ...
            if (reindexFlag.exists()) {
                boolean deleted = reindexFlag.delete();
                if (!deleted) {
                    log.error("Unable to delete Cris Configuration rebuild flag file " + reindexFlag.getAbsolutePath() +
                              ". You may need to delete it manually.");
                }
            }
        }
    ...
    public static String getCurrentFlywayState(Connection connection) throws SQLException {
        PreparedStatement statement =
            connection.prepareStatement("SELECT \"version\" FROM \"schema_version\" ORDER BY \"version\" desc");
