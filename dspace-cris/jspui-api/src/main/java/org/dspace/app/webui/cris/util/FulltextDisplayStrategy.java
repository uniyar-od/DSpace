...
public class FulltextDisplayStrategy extends ASimpleDisplayStrategy
        implements IAtomicDisplayStrategy
{
    ...
    public static final Log log = LogFactory
            .getLog(FulltextDisplayStrategy.class);
            ...
            boolean disableCrossLinks, boolean emph)
    {
        ...
        try
        {
            String normalizedValue = "none";
            String date = "";
            if(StringUtils.isNotBlank(value)){
            	if(StringUtils.startsWith(value, "embargo_restricted")) {
            		normalizedValue = "embargorestricted";
            		date = StringUtils.split(value,"_")[2];
            	}
            	else if (StringUtils.startsWith(value, "embargo")) {
            		normalizedValue = "embargo";
            		date = StringUtils.split(value, "_")[1];
            	}
            	else normalizedValue = value;
            }
            
            String metadataDisplay = MessageFormat.format(
                    I18nUtil.getMessage("jsp.display-strategy.fulltext." + normalizedValue,
                            UIUtil.obtainContext(hrq)),
                    hrq.getContextPath(), date);
