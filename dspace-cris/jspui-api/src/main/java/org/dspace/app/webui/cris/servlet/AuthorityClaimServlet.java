...
public class AuthorityClaimServlet extends DSpaceServlet
{
    ...
    private static String checksimilarityString;
    ...
    private ConfigurationService configurationService = dspace.getConfigurationService();
    ...
    private ChoiceAuthorityService choiceAuthorityService = ContentAuthorityServiceFactory.getInstance().getChoiceAuthorityService();

    private IndexingService indexer = dspace.getServiceManager()
            .getServiceByName(IndexingService.class.getName(),
                    IndexingService.class);
    
    public AuthorityClaimServlet() {
		forceCommit = configurationService.getBooleanProperty(CrisConstants.CFG_MODULE + ".publication.claim.list.solr.force.commit", false);
		checksimilarityString = configurationService.getProperty(CrisConstants.CFG_MODULE + ".publication.claim.list.checksimilarity");
	}
            ...
            throws SQLException, AuthorizeException, IOException
    {
        if(StringUtils.isNotBlank(metadata)) {
            ...
            // Group
            String notifyGroupSelfClaim = configurationService.getProperty("cris.notify-publication.claim.group.name");
            ...
            List<IMetadataValue> mmm = item.getMetadataWithoutPlaceholder(schema, element, qualifier, Item.ANY);
            itemService.clearMetadata(context, item, schema, element, qualifier, Item.ANY);
            for(IMetadataValue mm : mmm) {
                if(crisID.equals(mm.getAuthority())) {
                    templateEmailParamMetadataValue = mm.getValue();
                    templateEmailParamMetadataAuthority = mm.getAuthority();
                    templateEmailParamMetadataConfidence = ""+mm.getConfidence();
                    itemService.addMetadata(context, item, schema, element, qualifier,
                            mm.getLanguage(), mm.getValue(), null, Choices.CF_UNSET);
                }
                else {
                    itemService.addMetadata(context, item, schema, element, qualifier,
                            mm.getLanguage(), mm.getValue(), mm.getAuthority(), mm.getConfidence());
                }
            }
            ...
            throws SQLException, ServletException, IOException
    {
        ...
        for (Item ii : publications)
        {
            ...
            mapItem.put(handle, handleService.resolveToObject(context, handle));
            ...
            Item item, String field, String standardizeField)
    {
        ...
        for (IMetadataValue meta : metadatum)
        {
            String similar = null;
            choice: for (String allname : rp.getAllNames())
            {
                if (StringUtils.isNotBlank(allname) && (crisID.equals(meta.getAuthority())
                        || allname.equals(meta.getValue())
                        || jaroWinklerDistance.getDistance(allname,
                                meta.getValue()) > checksimilarity
                        || allname.startsWith(meta.getValue())
                        || meta.getValue().startsWith(allname)))
                {
                    similar = meta.getValue();
            ...
            String[] innerOptions = new String[] { meta.getValue(),
                    meta.getAuthority(), "" + meta.getConfidence(), meta.getLanguage(),
                    similar };
    ...
    private boolean isMemberOfSelfClaimGroup(Context context) throws SQLException
    {
        ...
        String nameGroupSelfClaim = configurationService.getProperty("cris.publication.claim.group.name");
    ...
    {
        List<UUID> selectedIds = UIUtil.getUUIDParameters(request, "selectedId");
    ...
    private void workNow(Context context, HttpServletRequest request,
            ...
            String notifyGroupSelfClaim, boolean selfClaim, UUID selectedId,
            String submitMode) throws SQLException, AuthorizeException
    {
        ...
        Item item = (Item) handleService.resolveToObject(context,
                handle);
            ...
            if(StringUtils.isNotBlank(choice)) {
                ...
                if (StringUtils.isNotBlank(fieldChoice))
                {
                    ...
                    else
                    {
                        ...
                        itemService.clearMetadata(context, item, metadata[1], metadata[2],
                        ...
                        if (gotoReview)
                        {
                            itemService.addMetadata(context, item, METADATA_MESSAGE[0],
                    ...
                    {
                        discard(context, crisID, fieldChoice, itemRejectedIDs, submitMode);
                    }
                }
    ...
    private void discard(Context context, String crisID,
            String fieldChoice, Set<UUID> itemRejectedIDs, String submitMode) throws SQLException, AuthorizeException
    {
        // notify reject
        ...
        choiceAuthorityService.notifyReject(ids, schema, element, qualifier, crisID);
    }

    private void findSelectedPersons(HttpServletRequest request, UUID selectedId,
            List<String> choices)
    {
            ...
            try
            {
                if (StringUtils.isNotBlank(emailUser))
                {
                    ...
                    if (StringUtils.isNotBlank(groupName)) {
                        ...
                        if (group != null && !groupService.isEmpty(group))
                        {
                ...
                email.addArgument(configurationService.getProperty("dspace.url")
                        + "/cris/rp/" + crisId);
                email.addArgument(configurationService.getProperty("dspace.url")
                        + "/handle/" + handle);
