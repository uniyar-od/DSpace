		    ...
		    if(context!=null) {
    			if (StringUtils.isNotBlank(uuid) && count>0) {
	                ...
	                if (metric.getTimeStampInfo() != null && metric.getTimeStampInfo().getTimestampCreated() != null)
	                {
	                    ...
	                    document.addField(metrictype+ConstantMetrics.STATS_INDICATOR_TYPE_TIME + separator + placeholder, fieldModifierAdditional); // add the map as the field value
	                }
	                if (metric.getStartDate() != null)
	                {
                        Map<String,Object> fieldModifierAdditional = new HashMap<>(1);
                        fieldModifierAdditional.put("set", metric.getStartDate());
	                    document.addField(metrictype+ConstantMetrics.STATS_INDICATOR_TYPE_STARTTIME + separator + placeholder, fieldModifierAdditional); // add the map as the field value
	                }
