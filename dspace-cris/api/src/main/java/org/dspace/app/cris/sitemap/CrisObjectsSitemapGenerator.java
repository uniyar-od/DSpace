...
public class CrisObjectsSitemapGenerator implements ISitemapGeneratorPlugin {
	...
	public String addUrls(Context c, boolean makeHTMLMap, boolean makeSitemapOrg, List<String> includes,
			AbstractGenerator html, AbstractGenerator sitemapsOrg) throws IOException {
		StringBuffer objectDetails = new StringBuffer();
		String crisURLStem = ConfigurationManager.getProperty("dspace.url") + "/cris/";
		...
		if (includes.contains("crispj")) {
			...
			addUrlsInternal(makeHTMLMap, makeSitemapOrg, html, sitemapsOrg, crisURLStem, crispjs);
		...
		if (includes.contains("crisou")) {
			...
			addUrlsInternal(makeHTMLMap, makeSitemapOrg, html, sitemapsOrg, crisURLStem, orgUnits);
		...
		if (includes.contains("crisrp")) {
			...
			addUrlsInternal(makeHTMLMap, makeSitemapOrg, html, sitemapsOrg, crisURLStem, crisrps);
		...
		for (DynamicObjectType dynType : dynTypes) {
			...
			if (includes.contains("cris" + dynShortName)) {
				long count = applicationService.countResearchObjectByType(dynType);
				// num of full pages plus 1 if there are residual items
				int numPages = (int) (count / 100) + (count % 100 > 0 ? 1 : 0);
				for (int page = 0; page < numPages; page++) {
					List<ResearchObject> researchers = applicationService.getResearchObjectPaginateListByType(dynType,
							"id", false, page, 100);

					addUrlsInternal(makeHTMLMap, makeSitemapOrg, html, sitemapsOrg, crisURLStem, researchers);
				}
				objectDetails.append(",cris").append(dynShortName).append("=").append(count);
			}
		}
		return objectDetails.toString();
	}

	private void addUrlsInternal(boolean makeHTMLMap, boolean makeSitemapOrg, AbstractGenerator html,
			AbstractGenerator sitemapsOrg, String crisURLStem, List<? extends ACrisObject> crisObjects)
			throws IOException {
		for (ACrisObject crisObj : crisObjects) {
			String url = crisURLStem + crisObj.getPublicPath() + "/" + crisObj.getCrisID();
