...
public class DiscoveryRestControllerIT extends AbstractControllerIntegrationTest {
    ...
    public void discoverFacetsTestWithoutParameters() throws Exception {
        ...
        getClient().perform(get("/api/discover/facets"))
            ...
            // We have 4 facets in the default configuration, they need to all be present in
    ...
    public void discoverFacetsAuthorTestWithSizeParameter() throws Exception {
        ...
        getClient().perform(get("/api/discover/facets/author")
            ...
            // The facetType has to be 'text' because that's how the author facet is
            // configured by default
            .andExpect(jsonPath("$.facetType", is("text")))
            // Because we've constructed such a structure so that we have more than 2 (size)
            // authors, there
            ...
            // Because there are more authors than is represented (because of the size
            // param), hasMore has to
            // be true
            // The page object needs to be present and just like specified in the matcher
            .andExpect(jsonPath("$.page",
                is(PageMatcher.pageEntry(0, 2))))
            // These authors need to be in the response because it's sorted on how many
            // times the author comes
    ...
    public void discoverFacetsAuthorWithAuthorityWithSizeParameter() throws Exception {
        ...
        getClient().perform(get("/api/discover/facets/author")
            ...
            // There always needs to be a self link
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/author")))
            // Because there are more subjects than is represented (because of the size
            // param), hasMore has to
            // be true
            // The page object needs to be present and just like specified in the matcher
            .andExpect(jsonPath("$.page",
                is(PageMatcher.pageEntry(0, 2))))
            // These subjecs need to be in the response because it's sorted on how many
            // times the author comes
            // up in different items
    ...
    public void discoverFacetsAuthorTestWithPrefix() throws Exception {
        ...
        getClient().perform(get("/api/discover/facets/author?prefix=smith")
            ...
            // configured by default
            .andExpect(jsonPath("$.facetType", is("text")))
            // We only request value starting with "smith", so we expect to only receive one
            // page
            ...
            // times the author comes
            // up in different items
            // These authors are order according to count. Only two show up because of the
            // prefix.
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(
                FacetValueMatcher.entryAuthor("Smith, Maria"),
    ...
    public void discoverFacetsAuthorTestForHasMoreFalse() throws Exception {
        // Turn of the authorization system so that we can create the structure
        // specified below
        context.turnOffAuthorisationSystem();
        ...
        // ** WHEN **
        // An anonymous user browses this endpoint to find the authors by the facets and
        // doesn't enter a size
        getClient().perform(get("/api/discover/facets/author"))
            ...
            .andExpect(jsonPath("$.name", is("author")))
            // The facetType has to be 'text' because that's the default configuration for
            // this facet
            .andExpect(jsonPath("$.facetType", is("text")))
            // There always needs to be a self link present
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/author")))
            // The page object needs to present and exactly like how it is specified here.
            // 20 is entered as the
            // size because that's the default in the configuration if no size parameter has
            // been given
            .andExpect(jsonPath("$.page",
                is(PageMatcher.pageEntry(0, 20))))
            // The authors need to be embedded in the values, all 4 of them have to be
            // present as the size
            // allows it
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(
                FacetValueMatcher.entryAuthor("Doe, Jane"),
    ...
    public void discoverFacetsAuthorTestForPagination() throws Exception {
        ...
        // We turn off the authorization system in order to construct the structure
        // defined below
        context.turnOffAuthorisationSystem();
        ...
        // The user enters a size of two and wants to see page 1, this is the second
        // page.
        getClient().perform(get("/api/discover/facets/author")
            .param("size", "2")
            .param("page", "1"))
            ...
            .andExpect(jsonPath("$.facetType", is("text")))
            // There needs to be a next link because there are more authors than the current
            // size is allowed to
            ...
            // There always needs to be a self link
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/author")))
            // The page object has to be like this because that's what we've asked in the
            // parameters
            .andExpect(jsonPath("$.page",
    ...
    public void discoverFacetsTestWithSimpleQueryAndSearchFilter() throws Exception {
        ...
        getClient().perform(get("/api/discover/facets/author")
            ...
            .andExpect(jsonPath("$.name", is("author")))
            // The facetType needs to be 'text' as that's the default configuration for the
            // given facet
            .andExpect(jsonPath("$.facetType", is("text")))
            // There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/author")))
            // The self link needs to contain the query that was specified in the
            // parameters, this is how it
            // looks like
            .andExpect(jsonPath("$._links.self.href", containsString("f.title=test,contains")))
            // The applied filters have to be specified like this, applied filters are the
            // parameters given
            ...
            .andExpect(jsonPath("$.page",
                is(PageMatcher.pageEntry(0, 20))))
            // These authors need to be present in the result because these have made items
            // that contain 'test'
            // in the title
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(
                FacetValueMatcher.entryAuthor("Smith, Donald"),
                FacetValueMatcher.entryAuthor("Testing, Works"))))
            // These authors cannot be present because they've not produced an item with
            // 'test' in the title
    ...
    public void discoverFacetsDateTest() throws Exception {
        // We turn off the authorization system in order to create the structure defined
        // below
        context.turnOffAuthorisationSystem();
        ...
        // ** WHEN **
        // An anonymous user browses this endpoint to find the dateIssued results by the
        // facet
        getClient().perform(get("/api/discover/facets/dateIssued"))
            ...
            // The facetType has to be 'date' because that's the default configuration for
            // this facet
            .andExpect(jsonPath("$.facetType", is("date")))
            // There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/dateIssued")))
            // This is how the page object must look like because it's the default with size
            // 20
            .andExpect(jsonPath("$.page",
                is(PageMatcher.pageEntry(0, 20))))
            // The date values need to be as specified below
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(
                // We'll always get atleast two intervals with the items specified above, so we
                // ask to match
                // twice atleast
    ...
    public void discoverFacetsTestWithScope() throws Exception {
        ...
        // ** WHEN **
        // An anonymous user browses this endpoint to find the author results by the
        // facet
        ...
        getClient().perform(get("/api/discover/facets/author")
            ...
            .andExpect(jsonPath("$.name", is("author")))
            // The facetType has to be 'text' as that's the default configuration for this
            // facet
            .andExpect(jsonPath("$.facetType", is("text")))
            ...
            // These are all the authors for the items that were created and thus they have
            // to be present in
        ...
        getClient().perform(get("/api/discover/facets/author")
            ...
            // These are the values that need to be present as it's ordered by count and
    ...
    public void discoverFacetsDateTestForHasMore() throws Exception {
        ...
        getClient().perform(get("/api/discover/facets/dateIssued")
            ...
            .andExpect(jsonPath("$.name", is("dateIssued")))
            // the facetType needs to be 'date' as that's the default facetType for this
            // facet in the
            // configuration
            .andExpect(jsonPath("$.facetType", is("date")))
            // There always needs to be a self link available
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/dateIssued")))
            // Seeing as we've entered a size of two and there are more dates than just two,
            // we'll need a next
            // link to go to the next page to see the rest of the dates
            .andExpect(jsonPath("$._links.next.href", Matchers.allOf(
                containsString("api/discover/facets/dateIssued"),
                containsString("page=1"))))
            // The page object needs to look like this because we've entered a size of 2 and
            // we didn't specify
            // a starting page so it defaults to 0
            .andExpect(jsonPath("$.page",
                is(PageMatcher.pageEntry(0, 2))))
            // There needs to be two date results in the embedded values section because
            // that's what we've
            // specified
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(
                FacetValueMatcher.entryDateIssued(),
    ...
    public void discoverFacetsDateTestWithSearchFilter() throws Exception {
        ...
        getClient().perform(get("/api/discover/facets/dateIssued")
            ...
            // There always needs to be a self link
            .andExpect(jsonPath("$._links.self.href", containsString("api/discover/facets/dateIssued")))
            // There needs to be an appliedFilters section that looks like this because
            // we've specified a query
            // in the parameters
            .andExpect(jsonPath("$.appliedFilters", containsInAnyOrder(
                AppliedFilterMatcher.appliedFilterEntry("title", "contains", "test", "test"))))
            // The page object needs to look like this because we entered a size of 2 and we
            // didn't specify a
            // starting page so it defaults to 0
            .andExpect(jsonPath("$.page",
                is(PageMatcher.pageEntry(0, 2))))
            // There needs to be only two date intervals with a count of 1 because of the
            // query we specified
            .andExpect(jsonPath("$._embedded.values", containsInAnyOrder(
    ...
    public void discoverSearchTest() throws Exception {
        ...
        getClient().perform(get("/api/discover/search"))
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // There needs to be a link to the objects that contains a string as specified
            // below
            ...
            // There needs to be a section where these filters as specified as they're the
            // default filters
    ...
    public void discoverSearchObjectsTest() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects"))
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // There needs to be a page object that shows the total pages and total elements
            // as well as the
            // size and the current page (number)
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 3))))
            // These search results have to be shown in the embedded.objects section as
            // these are the items
            ...
            .andExpect(jsonPath("$._embedded.searchResult._embedded.objects", Matchers.hasItems(
                ...
                SearchResultMatcher.match("core", "item", "items"),
                SearchResultMatcher.match("core", "item", "items"))))
            // These facets have to show up in the embedded.facets section as well with the
            // given hasMore
    ...
    public void discoverSearchObjectsTestHasMoreAuthorFacet() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects"))
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // The page object has to look like this because we've only made 7 elements (3
            // items),
            // the default size is 20 and they all fit onto one page (20 > 7) so totalPages
            // has
            // to be 1. Number is 0 because page 0 is the default page we view if not
            // specified otherwise
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 3))))
            // All elements have to be present in the embedded.objects section, these are
            // the ones we made in
            ...
            // property because we don't exceed their default limit for a hasMore true (the
            // default is 10)
            // We do however exceed the limit for the authors, so this property has to be
            // true for the author
            // facet
            .andExpect(jsonPath("$._embedded.facets", Matchers.containsInAnyOrder(
                FacetEntryMatcher.anyFacet("graphitemtype", "chart.pie"),
    ...
    public void discoverSearchObjectsTestHasMoreSubjectFacet() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects"))
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // The page object has to look like this because we've only made 7 objects (but
            // only 3 items),
            // they all fit onto 1 page because the default size is 20 and the default
            // starting page is 0.
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 3))))
            // All the elements created in the structure above have to be present in the
            ...
            // property because we don't exceed their default limit for a hasMore true (the
            // default is 10)
            // We do however exceed the limit for the subject, so this property has to be
    ...
    public void discoverSearchObjectsTestWithBasicQuery() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // The page object has to look like this because of the query we specified, only
            // two elements match
            // the query.
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 2))))
            // Only the two item elements match the query, therefore those are the only ones
            // that can be in the
            // embedded.objects section
            .andExpect(jsonPath("$._embedded.searchResult._embedded.objects", Matchers.containsInAnyOrder(
                SearchResultMatcher.match("core", "item", "items"),
                SearchResultMatcher.match("core", "item", "items"))))
            // We need to display the appliedFilters object that contains the query that
            // we've ran
            .andExpect(jsonPath("$.appliedFilters", contains(
                AppliedFilterMatcher.appliedFilterEntry("title", "contains", "test", "test"))))
            // These facets have to show up in the embedded.facets section as well with the
    ...
    public void discoverSearchObjectsTestWithScope() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // The page element has to look like this because it contains all the elements
            // we've just created
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 3))))
    ...
    public void discoverSearchObjectsTestWithDsoType() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // The page element needs to look like this and only have three totalElements
            // because we only want
            // the items (dsoType) and we only created three items
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 3))))
            // Only the three items can be present in the embedded.objects section as that's
            // what we specified
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // The page element needs to look like this and only have four totalElements
            // because we only want
            // the communities and the collections (dsoType) and we only created two of both
            // types
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 4))))
            // Only the two communities and the two collections can be present in the
            // embedded.objects section
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // The page element needs to look like this and only have five totalElements
            // because we only want
            // the collections and the items (dsoType) and we only created two collections
            // and three items
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 5))))
            // Only the two collections and the three items can be present in the
            // embedded.objects section
            // as that's what we specified in the dsoType parameter
            .andExpect(jsonPath("$._embedded.searchResult._embedded.objects", Matchers.containsInAnyOrder(
                SearchResultMatcher.match("core", "collection", "collections"),
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // The page element needs to look like this and have seven totalElements because
            // we want
            // the communities, the collections and the items (dsoType) and we created two
            // communities,
            // two collections and three items
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntryWithTotalPagesAndElements(0, 20, 1, 7))))
            // The two communities, the two collections and the three items can be present
    ...
    public void discoverSearchObjectsTestWithDsoTypeAndSort() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // The page object has to look like this and only contain three total elements
            // because we only want
            ...
            .andExpect(jsonPath("$._embedded.searchResult._embedded.objects", Matchers.containsInAnyOrder(
                ...
                SearchResultMatcher.match("core", "item", "items"),
                SearchResultMatcher.match("core", "item", "items"))))
            // Here we want to match on the item name in a certain specified order because
    ...
    // This test has been disable due to its innate dependency on knowing the
    // facetLimit
    ...
    public void discoverFacetsDateTestWithLabels() throws Exception {
        ...
        getClient().perform(get("/api/discover/facets/dateIssued"))
            ...
            // The type has to be 'discover'
            .andExpect(jsonPath("$.type", is("discover")))
            // The page object has to look like this because the default size is 20 and the
            // default starting
            // page is 0
            .andExpect(jsonPath("$.page", is(
                PageMatcher.pageEntry(0, 20))))
            // Then we expect these dateIssued values to be present with the labels as
            // specified
    ...
    public void discoverSearchObjectsTestForPaginationAndNextLinks() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            // These is the element that'll be shown (because page = 1, so the second
            // element
    ...
    public void discoverSearchObjectsTestWithContentInABitstream() throws Exception {
        ...
        // Run the filter media to make the text in the bitstream searchable through the
        // query
    ...
    // TODO Enable when solr fulltext indexing is policy-aware, see
    // https://jira.duraspace.org/browse/DS-3758
    @Test
    @Ignore
    public void discoverSearchObjectsTestWithContentInAPrivateBitstream() throws Exception {
        ...
        // Turn on the authorization again to make sure that private/inaccessible items
        // don't get show/used
        context.restoreAuthSystemState();
    ...
    public void discoverSearchObjectsTestForScopeWithPrivateItem() throws Exception {
        ...
        Collection col1 = CollectionBuilder.createCollection(context, child1).withName("Collection 1").build();
        Collection col2 = CollectionBuilder.createCollection(context, child1).withName("Collection 2").build();
        // 2. Two items that are readable by Anonymous with different subjects and one
        // private item
        Item publicItem1 = ItemBuilder.createItem(context, col1)
            .withTitle("Test")
            .withIssueDate("2010-10-17")
...
//                        SearchResultMatcher.matchOnItemName("item", "items", "Public item 2")
            ...
            // Make sure that the search result doesn't contain the item that's set to
            // private but does have
            // the correct scope
    ...
    public void discoverSearchObjectsTestForWorkspaceItemInCollectionScope() throws Exception {
        ...
        getClient(ePersonToken).perform(
            get("/api/discover/search/objects")
                // The workspace configuration returns all items (workspace, workflow, archived)
                // of the current user
                // see:
                // https://github.com/DSpace/RestContract/blob/main/search-endpoint.md#workspace
                .param("configuration", "workspace")
    ...
    public void discoverSearchObjectsTestForHitHighlights() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntry(0, 20))))
            // The search results has to contain the item with the query in the title and
            // the hithighlight has
    ...
    public void discoverSearchObjectsTestForHitHighlightsWithPrivateItem() throws Exception {
        ...
        Collection col1 = CollectionBuilder.createCollection(context, child1).withName("Collection 1").build();
        Collection col2 = CollectionBuilder.createCollection(context, child1).withName("Collection 2").build();
        // 2. Two public items that are readable by Anonymous with different subjects
        // and one private item
        Item publicItem1 = ItemBuilder.createItem(context, col1)
            .withTitle("Test")
            .withIssueDate("2010-10-17")
            .withAuthor("Smith, Donald")
    ...
    public void discoverSearchObjectsWithQueryOperatorContains_query() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            .andExpect(jsonPath("$._embedded.searchResult._embedded.objects", Matchers.containsInAnyOrder(
                ...
                SearchResultMatcher.matchOnItemName("item", "items", "Test 2"))))
            // These facets have to show up in the embedded.facets section as well with the
            // given hasMore property
            // because we don't exceed their default limit for a hasMore true (the default
            // is 10)
            .andExpect(jsonPath("$._embedded.facets", Matchers.containsInAnyOrder(
                FacetEntryMatcher.anyFacet("graphitemtype", "chart.pie"),
                FacetEntryMatcher.anyFacet("graphpubldate", "chart.bar"),
    ...
    public void discoverSearchObjectsTestForMinMaxValues() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            // We made 7 elements but only items are included in the dspace-cris default
            // configuration
            // -> 3 total elements
            .andExpect(jsonPath("$._embedded.searchResult.page", is(
                PageMatcher.pageEntryWithTotalPagesAndElements(1, 2, 2, 3))))
            // These are the two elements that'll be shown (because page = 1, so the third
            // element
    ...
    public void discoverSearchObjectsWithMissingQueryOperator() throws Exception {
        // ** WHEN **
        // An anonymous user browses this endpoint to find the the objects in the system
        // With the given search filter where there is the filter operator missing in
        // the value (must be of form
    ...
    public void discoverSearchObjectsWithNotValidQueryOperator() throws Exception {
        // ** WHEN **
        // An anonymous user browses this endpoint to find the the objects in the system
        // With the given search filter where there is a non-valid filter operator given
        // (must be of form
        // <:filter-value>,<:filter-operator> where the filter operator is one of:
        // “contains”, “notcontains”, "equals"
        // “notequals”, “authority”, “notauthority”, "query”); see enum
        // RestSearchOperator
        getClient().perform(get("/api/discover/search/objects")
    ...
    @Test
    /**
     * This test is intent to verify that inprogress submission (workspaceitem,
     * workflowitem, pool task and claimed tasks) don't interfers with the standard
     * search
     *
     * @throws Exception
     */
    public void discoverSearchObjectsWithInProgressSubmissionTest() throws Exception {
        ...
        // 3. three inprogress submission from a normal user (2 ws, 1 wf that will
        // produce also a pooltask)
        context.setCurrentUser(eperson);
        ...
        // ** WHEN **
        // An anonymous user, the submitter and the admin that browse this endpoint to
        // find the public objects in the
        ...
        for (String token : tokens) {
            getClient(token).perform(get("/api/discover/search/objects"))
                ...
                // These search results have to be shown in the embedded.objects section as
                // these are the items
                // given in the structure defined above (dspace-cris only return items by
                // default).
                // Seeing as everything fits onto one page, they have to all be present
                .andExpect(jsonPath("$._embedded.searchResult._embedded.objects", Matchers.containsInAnyOrder(
                    SearchResultMatcher.match("core", "item", "items"),
    ...
    @Test
    /**
     * This test is intent to verify that workspaceitem are only visible to the
     * submitter
     *
     * @throws Exception
     */
    public void discoverSearchObjectsWorkspaceConfigurationTest() throws Exception {
        ...
        // 3. three inprogress submission from our submitter user (2 ws, 1 wf that will
        // produce also a pooltask)
        WorkspaceItem wsItem1 = WorkspaceItemBuilder.createWorkspaceItem(context, col1).withTitle("Workspace Item 1")
        ...
        getClient(adminToken).perform(get("/api/discover/search/objects").param("configuration", "workspace"))
            ...
            // worfklowitem submitted by the admin user as by the structure defined above.
    ...
    @Test
    /**
     * This test is intent to verify that tasks are only visible to the appropriate
     * users (reviewers)
     *
     * @throws Exception
     */
    public void discoverSearchObjectsWorkflowConfigurationTest() throws Exception {
        ...
        // a simple collection
        // a second collection with 2 workflow steps that have 1 reviewer each
        ...
        // reviewer1 should see two pool items, one from the submitter and one from the
        // administrator
        // the other task in step1 is claimed by the administrator so it should be not
        // visible to the reviewer1
        getClient(reviewer1Token).perform(get("/api/discover/search/objects").param("configuration", "workflow"))
            ...
            // The claimed task of the administrator and the pool task for step 2 should not
            // be visible to
            // reviewer1.
        ...
        // because the admin is in the reviewer group for step 1, not because she is an
        // admin
        getClient(adminToken).perform(get("/api/discover/search/objects").param("configuration", "workflow"))
            ...
            // For step 1 one submitted by the user and one submitted by the admin and none
            // for step 2.
            // Seeing as everything fits onto one page, they have to all be present
            .andExpect(jsonPath("$._embedded.searchResult._embedded.objects", Matchers.containsInAnyOrder(
                Matchers.allOf(
    ...
    public void discoverSearchObjectsWorkflowAdminConfigurationTest() throws Exception {
        ...
        getClient(adminToken).perform(get("/api/discover/search/objects").param("configuration", "workflowAdmin"))
            ...
            // For step 1 one submitted by the user and one submitted by the admin and one
            // for step 2.
            // Seeing as everything fits onto one page, they have to all be present
            .andExpect(jsonPath("$._embedded.searchResult._embedded.objects", Matchers.containsInAnyOrder(
                Matchers.allOf(
    ...
    public void discoverSearchObjectsTestForWithdrawnOrPrivateItemsNonAdmin() throws Exception {
        ...
        String query = "Test";
        // ** WHEN **
        // A non-admin user browses this endpoint to find the withdrawn or private
    ...
    public void discoverSearchObjectsTestForWithdrawnOrPrivateItemsByAdminUser() throws Exception {
        ...
        String adminToken = getAuthToken(admin.getEmail(), password);
        // ** WHEN **
        // A system admin user browses this endpoint to find the withdrawn or private
        // objects in the system
        // With a query stating 'Test'
    ...
    public void discoverSearchObjectsTestForAdministrativeViewAnonymous() throws Exception {
        ...
        // An anonymous user browses this endpoint to find the withdrawn or private
        // objects in the system
        // With a query stating 'Test'
    ...
    public void relevanceByRelationPlacesTest() throws Exception {
        ...
        final Relationship publication2ToAuthor1 = RelationshipBuilder
            .createRelationshipBuilder(context, publication2, author1, selectedResearchOutput,
                -1, -1)
            .build();

        final Relationship publication1ToAuthor1 = RelationshipBuilder.createRelationshipBuilder(context, publication1,
            author1, selectedResearchOutput, -1, -1)
            .build();
        final Relationship publication3ToAuthor2 = RelationshipBuilder
            .createRelationshipBuilder(context, publication3, author2, selectedResearchOutput,
                -1, -1)
            .build();

        final Relationship publication2ToAuthor2 = RelationshipBuilder
            .createRelationshipBuilder(context, publication2, author2, selectedResearchOutput,
                -1, -1)
            .build();

        final Relationship patent1ToAuthor1 = RelationshipBuilder
            .createRelationshipBuilder(context, patent1, author1, selectedResearchOutput,
                -1, -1)
            .build();
    ...
    /**
     * This test checks the scenario when an item is related to many owners and it
     * is hidden by only one, many or all of them.
     * @throws Exception
     */
    @Test
    public void sameItemHiddenByDifferentOwners() throws Exception {
        ...
        final Relationship publicationOneHiddenByFirstPerson = RelationshipBuilder.createRelationshipBuilder(
        ...
        context.turnOffAuthorisationSystem();
        final Relationship publicationTwoHiddenBySecondPerson = RelationshipBuilder.createRelationshipBuilder(
        ...
        context.turnOffAuthorisationSystem();
        final Relationship publicationOneHiddenByProjectOwner = RelationshipBuilder
            .createRelationshipBuilder(context, publication1, project, researchOutputHiddenByProject)
            .build();
    ...
    public void discoverOrcidSearchLuckySearchConfigurationOneResult() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            .andExpect(jsonPath("$.appliedFilters[0].operator", is("equals")))
            // total elements needs to be 1 as there is only one item with the orcid
            // searched
            .andExpect(jsonPath("$._embedded.searchResult.page.totalElements", is(1)))
            .andExpect(jsonPath("$._embedded.searchResult._embedded.objects[0]._embedded.indexableObject.id",
                is(publicItem1.getID().toString())))
    ...
    public void discoverDoiSearchLuckySearchConfigurationMultipleResults() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            .andExpect(jsonPath("$.appliedFilters[0].operator", is("equals")))
            // the size of total elements needs to be three as there are three items found
            // from search
            .andExpect(jsonPath("$._embedded.searchResult.page.totalElements", is(3)))
    ...
    public void discoverOrcidSearchLuckySearchConfigurationNoResult() throws Exception {
        ...
        getClient().perform(get("/api/discover/search/objects")
            ...
            .andExpect(jsonPath("$.appliedFilters[0].operator", is("equals")))
            // total elements need to be 0 as there are not created items with the orcid
            // searched
            .andExpect(jsonPath("$._embedded.searchResult.page.totalElements", is(0)))
            .andExpect(jsonPath("$._embedded.searchResult._embedded.objects", hasSize(0)))
            .andExpect(jsonPath("$._links.next").doesNotExist())
